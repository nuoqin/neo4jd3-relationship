...
function Neo4jD3(_selector, _options) {
    var container, graph, info, node, nodes, relationship, relationshipOutline,
        relationshipOverlay, relationshipText, relationships, selector, simulation,
        svg, svgNodes, svgRelationships, svgScale, svgTranslate,
        classes2colors = {},
        justLoaded = false,
        numClasses = 0,
        options = {
    ...
    function appendOutlineToNode(node) {
        return node.append('circle')
            ...
            .style('stroke', function (d) {
                if (d.id.indexOf('user-') != -1) {
                    return '#2870c2';
                } else if (d.id.indexOf('project-') != -1) {
                    return '#52c41a';
                } else if (d.id.indexOf('factory-') != -1) {
                    return '#FA8C16';
                }
                return '#f36924';
     ...
     * @param {} node 
     */
    function hideOther(node) {
        document.querySelectorAll(".relationship").forEach(
            function (e) {
                let data = e.__data__
                debugger
                if (data.source.name != node.name) {
                    e.parentNode.removeChild(e);
                    removeOtherNode(data.source.name)
                }
                if (data.target.name != node.name) {
                    if(e.parentNode){
                        e.parentNode.removeChild(e);
                    }
                    removeOtherNode(data.target.name)
                }
                
            }
        );
    }
    function removeOtherNode(name) {
        document.querySelectorAll(".node").forEach(
            function (e) {
                if (e.__data__.name == name) {
                    e.parentNode.removeChild(e);
                }
            }
        );
     ...
     * @param {} node 
     */
    function hideCurrent(node) {
        //移除node数组
        //在移除关系
        removeCurrentNode(node.name)
        document.querySelectorAll(".relationship").forEach(
            function (e) {
                let data = e.__data__
                if (data.source.name == node.name) {
                    relationships=relationships.filter(item=>item.type!=data.type)
                    e.parentNode.removeChild(e);
                }
                if (data.target.name == node.name) {
                    relationships=relationships.filter(item=>item.type!=data.type)
    ...
    function removeCurrentNode(name) {
        
        document.querySelectorAll(".node").forEach(
            function (e) {
                if (e.__data__.name == name) {
                    if(nodes.length>0){
                         nodes=nodes.filter(item=>item.name!=name) 
                    }
                    e.parentNode.removeChild(e);
                }
            }
    ...
    function neo4jDataToD3Data(data) {
        ...
        data.data.forEach(function (data) {
            ...
            data.graph.relationships.sort(function (a, b) {
                let s1 = a.source.split("-")[1]
                let s2 = b.source.split("-")[1]
                let t1 = a.target.split("-")[1]
                let t2 = b.target.split("-")[1]
                if (s1 > s2) {
                    return 1;
                } else if (s1 < s2) {
                    return -1;
            ...
            setLinkGroup(data.graph.relationships)
        });
    ...
    function buildD3Data(d, data) {
        ...
        for (var i = 0; i < relationships.length; i++) {
            ...
            relationship.source = d.id
            relationship.target = relationship.endNode;
            relationship.linknum = s.relationships + 1 + i;
        }
    ...
    function clickNode(node) {
        //单击时圆圈上的字
        let relationshipOutline = svg.selectAll('.relationship .outline');
        relationshipOutline.attr("stroke-width", function (line) {
            console.log(line)
        ...
        relationshipOutline.attr("stroke", function (line) {
            console.log(line)
            if (line.source.name == node.name || line.target.name == node.name) {
                return "red";
            } else {
    ...
    function setLinkGroup(links) {
        let linkGroup = {};
        //对连接线进行统计和分组，不区分连接线的方向，只要属于同两个实体，即认为是同一组
        let linkmap = {};
        for (let i = 0; i < links.length; i++) {
            let source=links[i].source.split("-")[1], 
            target=links[i].target.split("-")[1]
            //这里处理应当放到后台 针对
            if(links[i].type=='主持'){
                links[i].transform="mark"
            }
            let key = source < target ? links[i].source + ':' + links[i].target:
    ...
    function tickRelationshipsTexts() {
        relationshipText.attr('transform', function (d) {
            var angle = (rotation(d.source, d.target) + 360) % 360,
                ...
                u = d.outline.u,
                middle = {
                    x: (source.x + target.x) / 2 + u.x * (10) * d.linkn + u.x,
                    y: (source.y + target.y) / 2 + u.y * (10) * d.linkn + u.y
                };
                if(d.transform=='mark'  && d.size>1){
                    if(target.y>0){
                        middle.y=-middle.y
                    }else{
                        middle.y=Math.abs(middle.y)
                    }   
                }
    ...
    function updateNodes(n) {
        let arr = []
        n.forEach(nd => {
            let f = nodes.filter(data => data.id == nd.id)
            if (f.length <= 0) {
                arr.push(nd)
            }
        })
    ...
    function updateRelationships(r) {
        ...
        let arr = []
        r.forEach(nd => {
            let f = relationships.filter(data => data.id == nd.id)
            if (f.length <= 0) {
                arr.push(nd)
            }
        })
